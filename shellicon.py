# -*- coding: utf-8 -*-
"""Іконки типів файлів — із самої Windows, а не з комплекту APSVN.
...
Два джерела, бо одного не вистачає:
* оболонка (`SHGetFileInfoW`) знає `.blend`, `.png`, `.psd`, `.fbx`, теки —
  усе, що зареєстроване в HKCR;
* Unreal Engine СВОЇ розширення в Windows не реєструє взагалі (перевірено:
  `.uasset`, `.umap`, `.uplugin` — «not registered»), тож для них іконку
  дістаємо з `UnrealEditor.exe`, знайденого через реєстр Epic.
...
# ------------------------------------------------------------------ фасад
def _generic():
    """Іконка, яку оболонка дає невідомому розширенню.

    Потрібна як еталон: якщо `.uasset` виглядає точно так само, значить
    оболонка нічого про нього не знає — і тоді має сенс лізти в редактор.
    """
    if "*" not in _cache:
        _cache["*"] = _sys_icon(".zzqq-nothing-claims-this")
    return _cache["*"]


def _build(ext):
    if ext == "<dir>":
        return _sys_icon("", folder=True)
    png = _sys_icon(ext)
    if ext in UE_EXT and (png is None or png == _generic()):
        exe = unreal_editor()
        if exe:
            png = _exe_icon(exe) or png
    return png
