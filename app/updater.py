...
#
# До версії 1.0.0 код лежав у корені збірки; з 1.1.0 він в app/. Клієнт старої
...
#
# Для САМОЇ 1.0.0 це запізно: вона перевіряє архів своїм старим списком
# (app.py і svn_client.py у корені) і нову розкладку не впізнає. Ці збірки
# треба один раз замінити вручну — далі оновлення піде саме.
...
def relaunch_target(staged, install):
    """Що запустити після підміни — судячи з того, що лежить у НОВІЙ збірці.

    Тут було зашито APSVN.bat, а з 1.1.0 його в збірці немає — є APSVN.exe.
    Підміна проходила б, а перезапуск ні: програма закривалася, і художник
    лишався з віконцем Windows «не можу знайти APSVN.bat» замість APSVN.
    Тому дивимося в збірку, яку ставимо, — той самий принцип, що й з
    ознаками розкладки: вирішує нова збірка, а не пам'ять старої.
    """
    if not desktop.WINDOWS:
        return install                    # .app відкривається цілком
    for name in ("APSVN.exe", "APSVN.bat"):
        if os.path.isfile(os.path.join(staged, name)):
            return os.path.join(install, name)
    return os.path.join(install, "APSVN.exe")
